...
export const MobileTradingLayout = () => {
  ...
  return (
    <div className="h-screen flex flex-col bg-background">
      ...
      <div className="flex absolute bottom-2 right-0  w-[90%] border-t border-border bg-background-card">
        <button
        ...
        <button
          ...
          className={`flex-1 py-3 text-sm ${activeTab === 'orderbook' ? 'text-white border-t-2 border-blue-500' : 'text-gray-500'}`}
        >
          Orderbook
        </button>
        <button
          onClick={() => setActiveTab('positions')}
          className={`flex-1 py-3 text-sm ${activeTab === 'positions' ? 'text-white border-t-2 border-blue-500' : 'text-gray-500'}`}
        >
          Positions
        </button>
        <button
